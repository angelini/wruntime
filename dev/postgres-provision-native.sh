...
set -euo pipefail
if [ "$#" -ne 2 ]; then
  echo "usage: dev/postgres-provision-native.sh <manifest-directory> <worktree>" >&2
  exit 2
fi
...
exec env WRT_WORKTREE_DEV_STATE_ROOT="$WRT_WORKTREE_DEV_STATE_ROOT" docker compose \
  --project-name "$WRT_COMPOSE_PROJECT_NAME" --project-directory "$worktree" -f "$worktree/docker-compose.yml" \
