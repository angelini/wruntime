...
Development setup is deliberately not a sandbox deployment workflow. Run `just dev-up` on the Docker-capable host from each worktree that needs a stack. State lives under `<absolute-git-dir>/wruntime-dev-state`; a Git-dir hash gives the project identity `wruntime-dev-<id>`, and an atomically claimed persistent slot gives PostgreSQL, RustFS, LGTM, and local runtime listeners a disjoint loopback port block. Containers, networks, volumes, PKI, fixture artifacts, and readiness records are therefore worktree-owned, while Docker may reuse the content-addressed provisioner image. Each preparation maps the Docker daemon architecture to `x86_64-unknown-linux-musl` or `aarch64-unknown-linux-musl`, verifies the mapped target is already installed, incrementally runs `cargo zigbuild` in that worktree's existing `target/`, verifies the ELF/static executable, and builds the staged two-file `Dockerfile`/`wr-cli` image. Compose consumes that exact image through a generated `pull_policy: never` override. The UID-70 provisioner reads only the combined public node bundle; private PKI directories and keys retain their sealed modes. `owner.json` and `fixture/ready.json` bind worktree identity, slot, endpoints, source, manifests, PKI, and complete build/image provenance and are published only after PostgreSQL, provisioning/migration, map reload, digest, and bucket validation. `just dev-up` reuses a compatible generation and destructively replaces missing, partial, or mismatched state in the same worktree before publishing readiness. Pi derives and consumes that worktree's published state and performs no Docker, reload, provision, migrate, or topology discovery. Local tests need no cross-worktree lock; protected deployment qualification remains separate and globally serialized on its shared remote targets.
