...
Database authorization is per namespace, not per module. The module `search_path` selects the default schema for unqualified SQL, while fully qualified access to another DB-enabled module schema in the same namespace is permitted by the namespace role. Cross-namespace access and access to `wr__jobs`/`wr_system` remain denied. Module schemas are admin-owned: the namespace role can create and use objects through grants but cannot drop a schema. Put schema DDL in versioned module migrations, not request handlers, and treat migration files as trusted admin-executed input rather than a schema sandbox. Exact types and signatures remain in [`wr-sdk/src/db.rs`](../../../wr-sdk/src/db.rs) and [`wit/db.wit`](../../../wit/db.wit).

Worker submissions have three distinct timing controls. The module's `request_timeout_secs` is for ordinary HTTP dispatch, `worker_job_timeout_secs` is the handler execution deadline and default queue lease, and an explicit job `timeout_secs` replaces only that row's fixed lease. Leases are not renewed. A lease shorter than legitimate handler execution can cause redelivery; fencing protects queue state from stale completion but not guest side effects, so worker handlers must be idempotent.
