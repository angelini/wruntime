...
Use `_router` directly when combining generated protobuf RPCs with custom responses. A configured proxy public alias validates JSON or form input and delivers protobuf wire bytes on the canonical `rpc_path`. A guest may decode that normalized request and deliberately emit JSON, HTML, redirects, or another representation. The [codegen coordinator](../../../examples/codegen/coordinator/src/lib.rs) demonstrates this pattern: its public aliases transcode requests to protobuf while the coordinator returns guest-owned JSON responses.
