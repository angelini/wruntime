...
- Changes to deployment generation, bundle integrity, remote lifecycle behavior, provider reset logic, deployment fixtures, or lifecycle assertions require `just validate-all --deployment-e2e` on the protected Proxmox runner. The runner owns three pinned disposable roles (manager A, manager B, node); manager B's VMID, reachable IP/SSH host, and baseline snapshot are mandatory protected inputs. Its node-agent fixture provisions the complete clean-VM baseline and initial cleanup retention policy before exercising the product's binary-only updater; updater invocations must contain no credential or host-topology arguments. A local explicit skip is not completion evidence for those changes.
