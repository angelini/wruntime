...
| Local supervisor or runner lifecycle | `just test-lifecycle-runners`; `cargo test -p wr-cli`; `just multi-node-inline`; warning-free ecommerce/stockmarket; static lifecycle-cruft audit |
...
- Supervisor/runner checks must leave no owned service process or active state socket after success. Intentional failure fixtures retain state only to prove diagnostics, then remove it explicitly. Remaining sleeps/polls in touched runners must be identified as bounded application, infrastructure, or provider waits; service readiness may not use them.
