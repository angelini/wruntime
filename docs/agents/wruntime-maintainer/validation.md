...
| Deployment generator, durable operation, node agent, or backend effect | relevant CLI/config/bundle/agent tests; `operation_test`, `node_agent_operation_test`, `manager_test`, `multi_manager_test`; deterministic output review; `just deployment-e2e-python-test`; `just validate-all --deployment-e2e` on the protected runner |
