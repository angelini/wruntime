...
- **Preserve topology:** `database.url` remains the platform job-queue URL. Generated `database.tenant` contains only the explicit private endpoint, stable installed certificate paths, and non-secret expected state. Every DB-enabled namespace has one certificate-authenticated clean-recycled pool sized by checked instance contributions; every worker entry has one non-pooled platform `LISTEN` session. One-shot readiness connections disconnect before runtime pools are built. Host `just dev-up` alone maps daemon architecture, incrementally builds the matching Linux-musl `wr-cli` in the owner's existing Cargo target, verifies and packages it through a two-file content-addressed UID-70 image with no pull, and owns development PKI/server/map/provision/migrate setup for the fixed `wruntime-dev` project. It publishes binary/context/image/base/toolchain-bound `owner.json` plus `fixture/ready.json` under `<absolute-git-common-dir>/wruntime-dev-state`. All linked worktrees derive that root, serialize on its lock inode, and verify scoped source/manifest/artifact/PKI digests before use; Pi/tests/examples never discover, repair, or create per-worktree setup. Only coordinated host `just dev-reprepare` may replace an incompatible generation.
...
- **Preserve:** one replay-stable manifest and shared policy validator bind the immutable authenticated principal/operation owner, ordered complete source/target sets, target generation/digest, deployment leaf, and every executable/image, backend-spec, config, credential, and selector digest. Exact Begin replay returns the original `PREPARED` receipt. `STAGING` and `FAILED_PRE_CLOSE` change no active selector. Only after exact source admission closure may a fenced host action atomically select `current-activation.json`; the stable launcher verifies the complete selected set. Owner-only phase compare-and-set is lease-free. Targets reach `READY_CLOSED` before remaining sources are selector-fenced, stopped, and masked. Opening requires exact typed source `STOPPED` evidence and target-only live membership. Digest mismatch and post-closure ambiguity become durable `FAILED_CLOSED`; no automatic rollback or generic cleanup is permitted.
