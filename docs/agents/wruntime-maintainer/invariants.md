...
- **Preserve runner ownership:** one persistent local supervisor retains every dev `Child`, detects unexpected exit, and reaps concurrent engine, proxy, then manager waves under separate 45-second external deadlines. Runners use typed lifecycle/deployment/health expectations rather than PID, TCP, logs, fixed sleeps, or expected-nonzero gates. Local and manager-deployment readiness must match a launcher-issued activation identity and exact service kind. State is removed only after proof of exit, and clean `down` is acknowledged only after socket/lock/state cleanup succeeds; a primary failure remains primary, while cleanup failure is separately reported and fails a clean run.
- **Inspect:** manager service/state/database and orchestration; proxy NodeService/routing/admission/listeners; engine `main.rs`/`server.rs`/`engine.rs`/`worker.rs`/`registry.rs`; lifecycle task ownership; `wr-cli/src/cmd/{dev,dev_supervisor,lifecycle,helpers}.rs`; example/deployment runners; and deployment generation.
- **Prove:** lifecycle, manager, health, proxy, version, worker, migration, multi-manager, CLI supervisor/expectation, and deployment-template tests; `just deployment-e2e-python-test`; warning-free local examples; and protected systemd/Docker lifecycle qualification when generation or remote lifecycle behavior changes.
