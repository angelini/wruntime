...
- **Preserve:** pre-commit cancel/failure/deadline enters deadline-free source restoration before terminal state. Commit occurs only after exact backend/lifecycle/routing evidence and per-slot authority convergence and is terminally successful. Release cleanup is independent per-node maintenance: periodic reconciliation alone discovers/materializes exact manager-derived revision/digest authority, every protection-changing transaction immediately fences existing authority, renewable generation leases cancel stale effects, and cleanup degradation never poisons serving health.
