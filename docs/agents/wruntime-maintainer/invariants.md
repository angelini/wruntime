...
- **Preserve:** manager-owned intent is immutable and idempotent by `(actor, request_token)`; one forward/restoration operation runs per node, with an absolute forward deadline, explicit phase, append-only events, and per-slot staged/committed authority. Agent claims, renewals, observations, and results are fenced by node, authenticated principal, activation ID, and lease epoch. Reported result fields are evidence only; the manager derives advancement from one coherent snapshot.
- **Preserve:** destructive authorization comes only from manager mTLS leaf fingerprints mapped to operators or node-bound agents. Source/proxy headers are never authorization. The agent attests exact protocol, canonical config/binary digests, backend, capabilities, and retention; protocol mismatch requires install/update remediation rather than compatibility behavior.
- **Preserve:** pre-commit cancel/failure/deadline enters deadline-free source restoration before terminal state. Commit occurs only after exact backend/lifecycle/routing evidence and per-slot authority convergence. Cleanup is visible committed work, uses a manager-derived revision/digest allow-list, and may be superseded only by a new rollback.
...
- **Prove:** migration, operation, node-agent-operation, manager mTLS, multi-manager takeover, lifecycle/proxy/version integration, deterministic bundle/config/unit tests, pure deployment assertions, and protected systemd/Docker qualification.
...
- **Preserve job administration:** every operation has explicit queue scope. Engines sharing one physical `wr__jobs` database use one stable queue ID; different databases use different IDs. The manager exposes `JobAdminService` only on a dedicated listener that trusts an operator-admin CA, manager-to-engine delegation uses a different CA, all three trust roots are pairwise disjoint, ordinary runtime principals cannot use the admin listener, and routing/source headers never authorize either hop. The manager stores only delegate metadata and never opens the queue database. Reads qualify delegates before dispatch; mutations qualify one deterministic delegate and never replay. List is metadata-only, bounded to 200, and uses filter-bound non-snapshot keyset cursors; inspect is the only byte-disclosing operation. Payload/result/error/metadata persistence and all transport hops share explicit compatible size ceilings. Persisted lifecycle corruption fails closed. Retry locks one dead row, resets a fresh attempt budget and all terminal/claim artifacts, preserves last failure, notifies before commit, and is never replayed after an uncertain response. Engine shutdown closes and drains admitted admin RPCs before route withdrawal.
