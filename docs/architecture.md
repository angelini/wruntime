...
Manager policy changes use a separate durable fleet rollout. `PREPARED`/`STAGING` create only digest-qualified binaries or OCI images, backend specs, immutable credential sets, owner-only `next.tmp`, and an unselected descriptor. The active config, credentials, unit/Compose selector, and `current-activation.json` remain source-owned, including after `FAILED_PRE_CLOSE`. After every source closes privileged admission, a fenced host action stops the service, promotes bounded `current.toml`/`previous.toml`, and atomically selects one descriptor containing the executable/image, backend spec, config, and credential paths plus digests. The stable launcher refuses any mismatch. Multi-manager execution retains a closed rollout-control endpoint; a sole manager may use only a fsynced, lease-authorized 120-second local continuation to reach `READY_CLOSED`. Failed-closed recovery is an explicit higher-generation successor, never automatic rollback.
