...
Queue administration preserves that ownership. An operator-admin client calls `JobAdminService` on the manager's dedicated mTLS listener, which trusts only the operator-admin CA. The manager resolves the explicit queue ID to one fresh, non-draining engine registration and delegates over a different manager-to-engine mTLS trust domain; it never opens the queue database. Read delegates are qualified with `CheckJobQueue` and may fail over only before operation dispatch. Mutations qualify exactly one deterministic delegate and never replay. The engine executes bounded list/summary/inspect SQL and the locked dead-only retry through its administrative pool. Engines sharing one physical database advertise one queue ID and are collapsed for discovery, avoiding double-counting. CA membership on the isolated listener—not `x-wr-*` metadata or an ordinary runtime principal—authorizes the call; the delegation CA prevents operator credentials from bypassing the manager. Runtime, operator-admin, and delegation CA roots must be pairwise disjoint at startup. Engine shutdown closes and drains job-admin admission before route withdrawal and deregistration.
...
Local foreground orchestration is owned by one scoped `wr-cli dev run` process; guest artifacts are built separately before the foreground run. It starts fixed waves—manager, named proxies concurrently, then engines concurrently—and validates launcher-issued activation identity plus exact service kind at every READY endpoint. After all engines are READY it captures the manager routing-table version and requires every exact proxy activation to report at least that installed version before launching the optional scenario. The scenario runs in its own session so interruption or service failure terminates its entire process group. Cleanup reaps concurrent engine, proxy, then manager waves; the 45-second termination-policy boundary latches failure evidence but the sole child owner stays alive in reap-only mode until exit is proven. Scenario failure remains primary, while cleanup failure is reported independently and makes a clean scenario fail. The command creates no supervisor socket, lock, or persistent process state. For deployed engines, the systemd unit or Docker Compose service remains the process owner and final-exit authority; lifecycle observation is not exit proof.

### Durable operator lifecycle

Remote lifecycle intent follows `operator → manager → node agent → systemd/Compose → process lifecycle`. `OperatorService` and `NodeAgentService` are served only on manager mTLS. A valid CA chain is not authorization: destructive access requires an explicitly configured leaf-certificate fingerprint mapped to an operator, while every agent fingerprint is bound to one node. HTTP source/proxy headers remain routing and observability metadata and never grant lifecycle authority.

The manager persists immutable request input, actor, rollout policy, an absolute forward deadline, per-slot progress, append-only events, and lease ownership. The installed agent first attests its activation ID, protocol, complete canonical host-policy digest, binary digest, backend, capabilities, and retention policy. Claims, renewals, observations, and results are fenced by `(node_id, agent_instance_id, lease_epoch)`. The agent receives only typed targets and effects; it never receives shell text or a PID. An expired lease, restarted agent, ambiguous delivery, or inspection error pauses forward work and requires actual backend evidence plus explicit resume rather than guessing or replaying an effect.

`wr_nodes.current_revision` is the committed serving snapshot and `target_revision` is the one staged snapshot. Exact per-slot authority permits source and target registrations to overlap while only the selected revision serves. Selection is atomic under `wr-node/slots/<slot>` and is not authority by itself. The manager advances each action only after its required evidence: digest-covered release metadata, backend identity/state, engine lifecycle identity, deregistration, route withdrawal/convergence, and fresh serving routes. It switches authority per slot, commits the deployment only after all required slots converge, then issues a manager-derived release-cleanup allow-list. Retention cleanup is visible operation work and can pause; it cannot undo a committed rollout.

Cancellation or forward-deadline expiry before commit irreversibly fences forward effects and enters deadline-free, lease-fenced source restoration before becoming terminal. A committed operation is not cancelled; rollback is a new durable operation and supersedes stale cleanup. Lifecycle stage, deployment availability, and backend running/exited evidence remain independent. `READY` alone cannot commit a rollout, endpoint absence is not lifecycle `STOPPED`, and operator drain does not call a lifecycle mutation. The backend adapter sends SIGTERM; the engine enters `STOPPING`, withdraws routes through `BeginEngineDrain`, waits for proxy convergence, quiesces, deregisters, and exits. The same systemd or Compose adapter then supplies typed final-exit evidence.
