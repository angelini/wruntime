...
Remote lifecycle intent follows `operator → manager → node agent → systemd/Compose → process lifecycle`. `OperatorService` authenticates an explicitly fingerprint-mapped viewer/operator. The manager persists immutable request input, actor, policy, per-slot progress, append-only events, and a fenced lease. A node-bound mTLS agent pulls one typed step; neither manager details nor CLI text become commands. Lease epoch and expiry fence stale agents, and host or lease uncertainty pauses rather than guessing.

`wr_nodes.current_revision` is the committed serving snapshot and `target_revision` is the one staged snapshot. Per-slot authority permits source and target overlap. Heartbeats and the background route monitor both require committed or explicit slot authority before publishing a route, so staging alone cannot cut traffic. Selection is atomic per slot under `wr-node/slots/<slot>`. Systemd and Compose keep stable service names and preserve SIGTERM plus the 45-second backend-owner boundary.

Lifecycle stage, deployment availability, and backend running/exited evidence are independent. In particular, `READY` is not a rollout commit gate by itself, endpoint absence is not lifecycle `STOPPED`, and operator drain does not call a lifecycle mutation. SIGTERM transitions the engine to `STOPPING`; its existing internal shutdown withdraws routes through `BeginEngineDrain`, waits for proxy convergence, quiesces, deregisters, and exits. The backend owner then supplies final-exit evidence.
