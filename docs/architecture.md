...
Local development uses one persistent `wr-cli dev` supervisor per state directory. It owns the actual Tokio child handles for every manager, proxy, and engine, rejects occupied lifecycle endpoints before spawn, requires every lifecycle observation to match a launcher-issued activation identity and service kind, and treats startup failure or unexpected exit as a terminal stack failure with retained evidence. Shutdown proceeds in concurrent waves—engines, then proxies, then manager—with a 45-second external deadline per wave and confirmed child reaping before supervisor state is removed. Short-lived `dev up`, `deploy`, `status`, `wait`, and `down` clients communicate with that owner over an owner-only Unix socket; PID files are not a supervision contract.
