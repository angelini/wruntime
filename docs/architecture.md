...
Manager policy changes use a separate durable fleet rollout. `PREPARED`/`STAGING` create only digest-qualified binaries or OCI images, backend specs, immutable credential sets, owner-only `next.tmp`, and an unselected descriptor. The active config, credentials, unit/Compose selector, and `current-activation.json` remain source-owned, including after `FAILED_PRE_CLOSE`. After every source closes privileged admission, fenced target actions promote bounded `current.toml`/`previous.toml` and atomically select one descriptor containing the executable/image, backend spec, config, and credential paths plus digests. Targets must reach `READY_CLOSED`, and the executor must prove and renew through an exact closed target control endpoint before selector-fencing, stopping, and masking source activations. The stable launcher refuses any mismatch. Completion requires the configured PostgreSQL lease view to contain no non-target manager, so a stopped source cannot complete while its membership lease remains fresh. A sole in-place manager transition may use only a fsynced, lease-authorized 120-second local continuation to reach `READY_CLOSED`. Failed-closed recovery is an explicit higher-generation successor, never automatic rollback.
