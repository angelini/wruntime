...
For development, `just dev-up` prepares isolated worktree-local PostgreSQL state and generated tenant configuration. Tests and examples consume its published endpoints without provisioning authority and verify the generation's internal bindings without requiring its source-at-preparation digest to match current source. Ordinary source changes therefore do not invalidate the fixture. A later host `just dev-up` converges provisioning and immutable migrations against retained volumes and never resets the database automatically; intentionally incompatible local state requires a manual reset. See [Testing](testing.md#development-services) for fixture behavior.
