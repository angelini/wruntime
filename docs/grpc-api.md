...
`wr-cli lifecycle status|wait --endpoint <url>` exposes this observation contract; `--tls` selects the CLI's manager mTLS credentials. For an authenticated manager response, `lifecycle status` additively projects `observation.privileged_admission` as exactly `OPEN`, `CLOSED_STARTUP`, `CLOSED_ROLLOUT`, or `CLOSED_MISMATCH`; malformed or unspecified manager admission fails closed, while non-manager status omits the field. `lifecycle wait` remains only an exact process-state expectation surface: waits require the expected service kind and activation identity and return distinct errors for transport/query failure, replacement, terminal-before-ready state, and timeout. These commands never interpret cluster health severity. Local process exit is proven by the foreground owner's retained `Child`; deployed process exit is proven by the node agent's typed Systemd process inspection, not by a lifecycle mutation RPC.
...
| `InfrastructureService.BeginManagerRollout` | infrastructure operator | Creates one principal-and-operation-owned manager rollout; exact request replay returns the original `PREPARED` receipt. Concurrent replacement owners, changed-payload token reuse, nonmonotonic policy, and an active failed guard are rejected. A matching one-use reset permit may be consumed only by the same principal. |
| `InfrastructureService.AdvanceManagerRollout` | recorded rollout owner | Lease-free phase compare-and-set. Manager-owned barriers require exact source closure, target `READY_CLOSED`, typed stopped evidence, target-only membership, and monotonic accepted policy before completion. Post-closure failure is durable `FAILED_CLOSED`. |
| `InfrastructureService.GetManagerRollout` | scoped read role | Returns the durable declaration, owner, phase, request/policy/manifest identity, and exact source/target members; it performs no recovery or host inference. |
| `InfrastructureService.ResetFailedManagerRollout` | recorded rollout owner | Compare-and-sets the exact `FAILED_CLOSED` rollout using complete role-qualified, conclusively stopped, uniform nonzero-policy evidence. Exact retry returns the stored receipt. Success clears only the active guard and installs one same-principal fresh-rollout permit; it neither changes accepted policy nor opens admission. |
...
| `NodeService.Attest` | node-bound `node-agent` | Persists claimed node/activation, process-computed binary digest, exact protocol, normalized advertised capabilities, authenticated principal, and manager receipt time. Compatibility requires exact binary/protocol plus `required capabilities ⊆ advertised`; sorted missing-capability diagnostics are stable. Local config and cleanup retention are not evidence. |
